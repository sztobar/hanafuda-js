hanafuda = () ->
	...
	turn = 1
	background
	freeslot1 = new FreeSlot()
	freeslot2 = new FreeSlot()
	lock = false
	win
	...
	menu =
		draw: () ->
			...
			Text.fillText(Text.pica, "Start", canvas.width / 2, 250)
		isClicked: (mx, my) ->
	...
	Win = (dealt, sets) ->
		...
		title = gamestate == 7 ? "Zwyci¿stwo" : gamestate == 8 ? "Remis" : "Przegrana"
		subtitle = gamestate == 7 ? "Wygra¿e¿" : gamestate == 8 ? "" : "Przeciwnik wygra¿"
		...
		if gamestate != 8
			...
			dealt.y = 335
			Text.score()
			for i in sets.length # 80, 170
				ctx.fillText(sets[i], 80, 200 + (20 * i))
			ctx.fillText("£acznie : " + dealt.mypoints + " pkt", 260, 200)
			#ctx.fillText(dealt.mypoints, 260, 190)
			dealt.draw()

	class card
		constructor: (mo, ty, ix, iy) ->
			month = mo
			type = ty
			imgx = ix
			imgy = iy
	...
	deck = () ->
		...
		ret =
			...
			draw: () ->
				ctx.drawImage(cardback, this.x, this.y, this.width, this.height)
				if this.freecard not undefined
					this.freecard.draw()
			stroke: () ->
			...
			click: (mx, my) ->
				if gamestate is 1
					...
					gamestate = 4
					draw()
					#@freeslot.attach(card, this)
			pop: () ->
				return cards.pop()
			isClicked: (mx, my) ->
				if x >= @x and mx <= (@x + @width) and my >= @y and my <= (@y + @height)
					@click(mx, my)
	()
		...
		return {
			...
			draw: function () {
				...
				case 0:
					break;
				case 1:
					this.text = "Wybierz kartê™ z rêki™i.";
					break;
				case 2:
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 * Rozdanie kart
	 */
	function deal(firstplayer, secondplayer) {
		delayfun(function () {
			firstplayer.add(deck.pop());
			firstplayer.add(deck.pop());
			setTimeout(function () {
				table.add(deck.pop());
				table.add(deck.pop());
